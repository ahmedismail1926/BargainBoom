...
exports.createOffer = async (req, res, next) => {
  try {
    ...
    if (product.isAuction) {
      return res.status(400).json({ message: 'Cannot make counter offers on auction items' });
    }
    if (product.sellerId.toString() === req.user._id.toString()) {
      return res.status(400).json({ message: 'Sellers cannot make offers on their own products' });
    }
...
exports.updateOfferStatus = async (req, res, next) => {
  try {
    ...
    const product = await productService.getProductById(offer.productId);
    if (!product) {
      return res.status(404).json({ message: 'Product not found' });
    }
    ...
    if (status === 'countered' && !counterPrice) {
      return res.status(400).json({ message: 'Counter price is required when countering an offer' });
    }
    
    const updatedOffer = await offerService.updateOfferStatus(offerId, status, counterPrice);
